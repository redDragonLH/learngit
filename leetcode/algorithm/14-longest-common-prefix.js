 ...
 * 官方题解：二分法
 * 
 * 思路： 最长公共前缀的长度不会超过最短字符串的长度，用 minLength 表示字符串数组中的最短字符串的长度，
 * 则可以在[0,minLength]的范围内通过二分查找得到最长公共前缀的长度。每次去查找范围的中间值 mid ,判断每个字符串的长度为 mid 的前缀是否相同，
 * 如果相同则最长公共前缀的长度一定大于或等于mid,如果不相同则最长公共前缀的长度一定小于mid
 */
/**
 * 二分法 java 代码
 * 
 ...
 *    String str0 = strs[0].subsring(0, length);
